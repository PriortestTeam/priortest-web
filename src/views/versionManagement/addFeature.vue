          ...
          >
            <div>
              ...
              <el-button
                v-if="featureForm.id && isEdit"
                type="primary"
                @click="submitForm('featureForm')"
                >确认修改
              </el-button>
              <el-button type="primary" @click="giveupBack('featureForm')"
                >放弃</el-button
              >
              ...
              <el-row>
                ...
                >
                  ...
                  >
                    ...
                    <el-link
                      v-if="field.fieldType === 'link' && !isEdit"
                    ...
                    >
                      <el-option
                        v-for="item in handleOptions(
                          field.possibleValue,
                          field.fieldType === 'linkedDropDown'
                        )"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
...
import { featureSave, featureUpdate, featureInfo } from "@/api/feature";
...
export default {
  ...
  computed: {
    featureForm() {
      try {
        return [...this.sysCustomFields, ...this.customFields].reduce(
          (a, b) => {
            return {
              ...a,
              [b.label]: b.valueData,
  ...
  methods: {
    ...
    getData() {
      ...
      getAllCustomField({
        ...
        if (res.code === "200") {
          ...
          const data = res.data;
          this.sysCustomFields = data
            .filter((item) => item.type === "sField")
            .map((item, index) => {
          ...
          if (this.id) {
            featureInfo({ id: this.id }).then((res) => {
              ...
              fields.forEach((item, index) => {
                ...
                if (
                  Object.keys(featureExpand).length &&
                  featureExpand.attributes.find(
                    (o) => o.customFieldLinkId === item.customFieldLinkId
                  )
                ) {
                  item.valueData = featureExpand.attributes.find(
                    (o) => o.customFieldLinkId === item.customFieldLinkId
                  ).valueData;
                }
              });
    ...
    handleOptions(obj, flag) {
      ...
      try {
        ...
        if (flag) {
          const list = [];
          const parent = [...that.sysCustomFields, ...that.customFields].find(
            (item) => item.customFieldId === obj.others.parentListId
          );
    ...
    hasParentFiled(filed) {
      if (filed.fieldType === "linkedDropDown") {
        ...
        const index = [...this.sysCustomFields, ...this.customFields].findIndex(
          (item) => item.customFieldId === possibleValue.others.parentListId
        );
        return index > -1;
      }
      return true;
