        ...
        <el-card>
          <div class="project_table">
            ...
            <el-form>
              ...
              <div class="state"><span>状态</span><el-select v-model="value" placeholder="请选择">
            ...
            <div v-loading="isLoading" class="table protable">
              <el-table ref="featuretableData" :data="featuretableData" :header-cell-style="tableHeader" stripe
                ...
                <el-table-column prop="updateTime" label="关联日期" min-width="120" :show-overflow-tooltip="true" />

                <el-table-column label="操作" min-width="148" fixed="right">
                  <template slot-scope="scope">


                    <el-button type="text" class="table-btn" @click.stop="delproject(scope.row.id)">移除
                    </el-button>
                  </template>
                </el-table-column>
              </el-table>
...
import { getBeanFeatureList, getBeanIssueList, getBeanTestCaseList, getBeanTestCycleList } from '@/api/bean'
...
export default {
  ...
  created() {
    ...
    getAllCustomList({ projectId: localStorage.getItem('projectId'), scopeId: 9000001 }).then(res => {
      if (res.code === '200') {
  ...
  methods: {
    ...
    getqueryBeanList() {
      ...
      getBeanFeatureList().then(async res => {
        if (res.code === '200') {
      ...
      // })
      getBeanIssueList().then(async res => {
        if (res.code === '200') {
      ...
      getBeanTestCaseList().then(async res => {
        if (res.code === '200') {
          console.log("res-bean: ", res);
        }
        this.isLoading = false
      ...
      getBeanTestCycleList().then(async res => {
        if (res.code === '200') {
          console.log("res-bean: ", res);
        }
        this.isLoading = false
