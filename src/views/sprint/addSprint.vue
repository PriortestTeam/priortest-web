      ...
      <el-tabs v-model="activeName" type="card" :before-leave="handBeforeLeave">
        <el-tab-pane label="新建" name="first">
          ...
          >
            <div>
              <el-button
                v-if="!sprintForm.id && isEdit"
                type="primary"
                @click="submitForm('sprintForm', false)"
              >保存&新建
              </el-button>
              <el-button
                v-if="!sprintForm.id && isEdit"
                type="primary"
                @click="submitForm('sprintForm', true)"
              >保存&返回
              </el-button>
              <el-button
                v-if="!sprintForm.id && isEdit"
              ...
              <el-button v-if="sprintForm.id && isEdit" type="primary" @click="submitForm('sprintForm')">确认修改
              </el-button>
              <el-button type="primary" @click="giveupBack('sprintForm')">放弃&返回</el-button>
              <router-link v-if="!sprintForm.id" to="/admincenter/admincenter">
                <el-button type="text">
        ...
        <el-tab-pane label="链接" name="second">
          <add-link v-if="!verification(id)" :case-id="id" />
        </el-tab-pane>
    ...
    submitForm(formName, type) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.id) {
            sprintUpdate({ id: this.id, ...params })
              .then((res) => {
                if (res.code === '200') {
