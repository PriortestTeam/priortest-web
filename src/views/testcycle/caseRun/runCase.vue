    ...
    <el-table :data="useCaseData">
      ...
      <el-table-column label="执行">
        <template slot-scope="scope">
          ...
          <el-button @click="executeStep(scope.$index, scope.row, $event, tcaseInTcycleList[tcaseListIndex])"
            class="custom-button fail">失败&缺陷（自动）</el-button> |
...
.container {
  ...
  /* Gap between buttons */
  flex-wrap: wrap;
  /* Allow buttons to wrap when container width is insufficient */
  margin-bottom: 10px;
  /* Optional margin to separate from the table */
}
