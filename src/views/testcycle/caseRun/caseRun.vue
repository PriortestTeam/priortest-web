    ...
    <div class="Drawer" v-if="visible">
      ...
      <div class="list">
        ...
        <template v-for="(item,index) in DrawerList">
          <div class="item" @click="changeDrawerindex(index,'change')"  :key="index" :style="{backgroundColor: (Drawerindex == index? '#eee': '#fff')}">
            {{item}}
          </div>
        </template>
      </div>
    ...
    <div class="content" :style="{width:(visible ? '1560px':'1902px')}">
      <div class="title">
        ...
        <div class="title-down">
          ...
          <el-button style="width: 70px" @click="changeDrawerindex(Drawerindex,'up')" type="success">用例翻动（up - down)</el-button>
      ...
      <div class="table">
        ...
        <el-table
          :header-cell-style="{background:'#4286CD',color:'#fff'}"
          border
          :data="useCaseData"
          style="width: 100%">
          <el-table-column
          ...
          <el-table-column label="执行" width="530">
            <template slot-scope="scope">
              ...
              <el-button type="text" style="color: #DF3D66">失败&缺陷（自动）</el-button>|
              <el-button @click="handleFailIssue(scope.$index, scope.row)" type="text" style="color: #FF0000">失败&缺陷</el-button>|
  ...
  export default {
    ...
    data () {
      return {
        ...
        DrawerList: ['ID     测试用例标题        运行状态','第二条','第三条','第四条','第五条','第六条','第七条','第二条','第三条','第四条','第五条','第六条','第七条'],
        Drawerindex: 0,
        // 第一个table数据
        useCaseData: [{
          Step: '打开测试链接: Http://www.google.com',
          TestData:"Url Http://www.google.com",
          ExpectedResult: '用户可以打开页面',
          ActualResult: '可输入 您的实际测试结果'
        },{
          Step: '输入查询关键字：测试， 点击查询按钮',
           TestData:"测试",
          ExpectedResult: '用户可以看到返回查询结果',
          ActualResult: '可输入 您的实际测试结果'
        }],
